...
echo "\n";
echo color("red","                    HILMAWAN TUBE \n");
echo color("white","           Time  : ".date('[d-m-Y] [H:i:s]')."   \n");
echo color("green","                  SCRIFT GOJEK AGUSTUS      \n");
echo color("yellow","                 AUTO CLAIM VOCER 15K\n");
// function change(){
        $nama = nama();
        $email = str_replace(" ", "", $nama) . mt_rand(100, 999);
        echo color("yellow"," [!]NOMOR FRESH: ");
        // $no = trim(fgets(STDIN));
        ...
        otp:
        echo color("purple"," [?] Otp Bro : ");
        $otp = trim(fgets(STDIN));
        $data1 = '{"client_name":"gojek:cons:android","data":{"otp":"' . $otp . '","otp_token":"' . $otptoken . '"},"client_secret":"83415d06-ec4e-11e6-a41b-6c40088ab51e"}';
        $verif = request("/v5/customers/phone/verify", null, $data1);
        if(strpos($verif, '"access_token"')){
        echo color("green","[✓] Sudah Siap Bos\n");
        $token = getStr('"access_token":"','"',$verif);
        $uuid = getStr('"resource_owner_id":',',',$verif);
        echo color("red"," [!] Akses Token Lo: ".$token."\n\n");
        save("token.txt",$token);
        echo color("yellow","\n!!!!=======RAKUS NGADUKAN DAMAGE======!!!!");
        ...
        echo "\n".color("read","[!]KAMU PUNYA INI ".$total." : ");
